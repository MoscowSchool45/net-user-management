        ...
        "letsencrypt": [
            {
                ...
                "filename": "letsencrypt.eline.conf",
        ...
        "nginx": [
            {
                ...
                "filename": "app.eline.include",
                "remote_path": "/etc/nginx/conf.d/%(filename)s"
            },
            {
                "pattern": "app.eline.proxy_django.include",
                "filename": "app.eline.proxy_django.include",
                "remote_path": "/etc/nginx/conf.d/%(filename)s"
            },
            {
                "pattern": "%(target)s.ssl",
                "filename": "ssl.eline.include",
                "remote_path": "/etc/nginx/conf.d/%(filename)s"
            },
            {
                "default_site": True,
...
def test():
    ...
    defaults()
    env.hosts = ['test.accounts.ms45.edu.ru']
...
def live():
    ...
    env.hosts = ['production.accounts.ms45.edu.ru']
...
def setup_server(id=None):
    ...
    # Create database
    sudo('echo "CREATE DATABASE eline; '
         '      CREATE USER eline WITH password \'{db_password}\'; '
         '      GRANT ALL PRIVILEGES ON DATABASE eline to eline;" '
         '| docker exec -i postgres-10 psql -U postgres'.format(db_password=db_password))

    # Upload local settings
    put(local_path=StringIO(local_settings), remote_path=env.code_dir + '/eline/settings/local.py', use_sudo=True)

    # Create log dir
    sudo('mkdir -p /var/log/eline/')
    ...
    sudo('touch %s/eline/app/webpack-stats.json' % env.code_dir)
    ...
    # Copy logrotate conf
    with cd(env.code_dir):
        sudo('cp deploy/logrotate.conf /etc/logrotate.d/eline')
...
def collectstatic(npm_build=True):
    if npm_build:
        docker_compose_run('node', 'npm run build', name='eline_npm_build')
...
def docker_compose_run(service, cmd='', name='eline_tmp'):
...
def ensure_docker_networks():
    ...
    ensure_docker_network_exists('eline_default', [], internal=False)
    ensure_docker_network_exists('eline_nginx', ['nginx'])
    ensure_docker_network_exists('eline_postgres', ['postgres-10'])
